...
if __name__ == "__main__":
    ...
    path_plastic = parent_dir + "/UR_experiments/bags/plastic_linear_t5_2cm_1.bag"


    timestamps_traj, data_traj = read_rosbag_trajectory_data(path_case)
    time0 = timestamps_traj[0]-1
    timestamps_traj -= time0
    timestamps, data = read_rosbag_lindist_data(path_case)
    timestamps -= time0
    timestamps_fn, data_fn = read_rosbag_fn_data(path_case)
    timestamps_fn -= time0

    timestamps_traj_plast, data_traj_plast = read_rosbag_trajectory_data(path_plastic)
    time0 = timestamps_traj_plast[0]-1
    timestamps_plastic, data_plastic = read_rosbag_lindist_data(path_plastic)
    timestamps_plastic -= time0
    timestamps_plastic_fn, data_plastic_fn = read_rosbag_fn_data(path_plastic)
    timestamps_plastic_fn -= time0

    f, (ax1, ax2) = plt.subplots(2, 1, figsize=(6,4))

    ax1.plot(timestamps_plastic, data_plastic, alpha=0.7, linewidth=2, label="Plastic")
    ax1.plot(timestamps, data, alpha=0.7, linewidth=2, label="Case")
    ax1.plot(timestamps_traj, data_traj, alpha=0.7, linewidth=2, label="Trajectory")
    ...
    ax2.plot(timestamps_plastic_fn, data_plastic_fn, alpha=0.7, linewidth=2, label="Plastic")
    ax2.plot(timestamps_fn, data_fn, alpha=0.7, linewidth=2, label="Case")

    ax2.get_yaxis().set_label_coords(-0.07,0.2)
    ax2.set_ylabel('$f_n$ [N]')
