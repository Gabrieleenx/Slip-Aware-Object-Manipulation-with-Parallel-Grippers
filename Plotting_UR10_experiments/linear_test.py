...
if __name__ == "__main__":
    ...
    path_plastic = parent_dir + "/UR_experiments/bags/plastic_linear_t5_2cm_2.bag"
    path_wood = parent_dir + "/UR_experiments/bags/wood_linear_t5_2cm_1.bag"
    path_cardboard = parent_dir + "/UR_experiments/bags/cardboard_linear_t5_2cm_1.bag"
    path_case = parent_dir + "/UR_experiments/bags/case_linear_t5_2cm_1.bag"

    path_plastic2 = parent_dir + "/UR_experiments/bags/plastic_linear_t2_4cm_1.bag"
    path_wood2 = parent_dir + "/UR_experiments/bags/wood_linear_t2_4cm_1.bag"
    path_cardboard2 = parent_dir + "/UR_experiments/bags/cardboard_linear_t2_4cm_1.bag"
    path_case2 = parent_dir + "/UR_experiments/bags/case_linear_t2_4cm_1.bag"


    timestamps_traj, data_traj = read_rosbag_trajectory_data(path_plastic)
    time0 = timestamps_traj[0]-1
    timestamps_traj -= time0
    timestamps, data = read_rosbag_lindist_data(path_plastic)
    timestamps -= time0

    timestamps_con, data_con = read_rosbag_control_pos_data(path_plastic)
    timestamps_con -= time0

    timestamps_traj_, data_traj_ = read_rosbag_trajectory_data(path_wood)
    time0 = timestamps_traj_[0]-1
    timestamps_wood, data_wood = read_rosbag_lindist_data(path_wood)
    timestamps_wood -= time0

    timestamps_traj_, data_traj_ = read_rosbag_trajectory_data(path_case)
    time0 = timestamps_traj_[0]-1
    timestamps_case, data_case = read_rosbag_lindist_data(path_case)
    timestamps_case -= time0

    timestamps_traj_, data_traj_ = read_rosbag_trajectory_data(path_cardboard)
    time0 = timestamps_traj_[0]-1
    timestamps_cardboard, data_cardboard = read_rosbag_lindist_data(path_cardboard)
    ...
    ax1.plot(timestamps_wood, data_wood, alpha=0.7, linewidth=1.5, label="Wood")
    ...
    ax1.set_ylabel('$|\hat{x}|$ [mm]')

    ax_inset = inset_axes(ax1, width="20%", height="50%", bbox_to_anchor=(-0.05, -0.28, 1, 1),
                      bbox_transform=ax1.transAxes)
    ax_inset.plot(timestamps, data, alpha=0.7, linewidth=1.5, label="Plastic")
    ax_inset.plot(timestamps_wood, data_wood, alpha=0.7, linewidth=1.5, label="Wood")
    ax_inset.plot(timestamps_case, data_case, alpha=0.7, linewidth=1.5, label="Case")
    ax_inset.plot(timestamps_cardboard, data_cardboard, alpha=0.7, linewidth=1.5, label="Cardboard")
    ax_inset.plot(timestamps_traj, data_traj, alpha=1.0, linewidth=2, label="Trajectory")

    # Set the zoomed area
    x1, x2, y1, y2 = 3.60, 3.68, 8, 14  # Specify the region you want to zoom into
    ax_inset.set_xlim(x1, x2)
    ax_inset.set_ylim(y1, y2)

    # Add a rectangle in the main plot to show the zoom area
    #ax1.indicate_inset_zoom(ax_inset)
    mark_inset(ax1, ax_inset, loc1=2, loc2=3, fc="none", ec="0.5")
    ...
    time0 = timestamps_traj[0]-1
    timestamps, data = read_rosbag_lindist_data(path_plastic2)
    timestamps -= time0
    timestamps_traj -= time0

    timestamps_con, data_con = read_rosbag_control_pos_data(path_plastic2)
    timestamps_con -= time0

    timestamps_traj_, data_traj_ = read_rosbag_trajectory_data(path_wood2)
    time0 = timestamps_traj_[0]-1
    timestamps_wood, data_wood = read_rosbag_lindist_data(path_wood2)
    timestamps_wood -= time0


    timestamps_traj_, data_traj_ = read_rosbag_trajectory_data(path_case2)
    time0 = timestamps_traj_[0]-1
    timestamps_case, data_case = read_rosbag_lindist_data(path_case2)
    timestamps_case -= time0

    timestamps_traj_, data_traj_ = read_rosbag_trajectory_data(path_cardboard2)
    time0 = timestamps_traj_[0]-1
    timestamps_cardboard, data_cardboard = read_rosbag_lindist_data(path_cardboard2)
    timestamps_cardboard -= time0

 
    ax2.plot(timestamps, data, alpha=0.7, linewidth=1.5, label="Plastic")
    ax2.plot(timestamps_wood, data_wood, alpha=0.7, linewidth=1.5, label="Wood")
